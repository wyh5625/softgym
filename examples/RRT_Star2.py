...
class RRTStar:
    ...
    def select_grasp_layer_for_expansion2(self):
        # calculate total number of states in all layers
        total_states = sum(len(states) for states in self.grasp_layer.values())

        if total_states == 0:
            return None

        # calculate the relative number of states in each layer as a weight
        weights = [len(states)/total_states for states in self.grasp_layer.values()]
    ...
    def find_path(self):
        ...
        for i in range(1, self.max_iter + 1):
            ...
            states_with_unused_controls = [state for state in grasp_layer_to_expand if self.unused_controls[state]]
            # print("num of states with unused controls: ", len(states_with_unused_controls))
            if not states_with_unused_controls:
                continue

            # The state to expand is the one that is closest to the random_node
            state_to_expand = min(states_with_unused_controls, key=lambda state: self.distance(state.node, random_node))
            ...
            if unreached_states and random.random() < (1-unused_ratio) *self.rpho:
