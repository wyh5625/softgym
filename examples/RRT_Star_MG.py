...
class MG_RRTStar:
    ...
    def set_constraints(self, table_center=(0,-0.9), table_size=(1,1), reachable_length = 1.1, object_size=(0.6, 0.36)):
    ...
    def find_u_and_delta_s(self, s_1, s_2, cp_id):
        ...
        P = coordinate_to_matrix(self.cp_dict[cp_id][0], self.cp_dict[cp_id][1], self.cp_dict[cp_id][2])
        P_1 = np.dot(T_1, P)
        P_2 = np.dot(T_2, P)
    ...
    def populate_control_space(self, cs_file):
        ...
        for idx, g_key in enumerate(grouped.groups.keys()):
            ...
            self.cp_dict[idx] = list(g_key)
            print(g_key)
            for index, row in group.iterrows():
                self.U.append((idx, [row['trans_x'], row['trans_y'], row['rot']]))
        ...
        for i in range(len(self.U)):
            ...
            T_w_2_p = coordinate_to_matrix(self.cp_dict[cp_id][0], self.cp_dict[cp_id][1], self.cp_dict[cp_id][2])
    ...
    def distance(self, pos_1, pos_2):
        ...
        return math.sqrt((s1_tilde[0] - s2_tilde[0]) ** 2 + (s1_tilde[1] - s2_tilde[1]) ** 2 + (s1_tilde[2] - s2_tilde[2]) ** 2)
    ...
    def f(self, s, u):
        cp_id = u[0]
        T_s_2_p = coordinate_to_matrix(self.cp_dict[cp_id][0], self.cp_dict[cp_id][1], self.cp_dict[cp_id][2])
    ...
    def steer(self, s1, s2):
        ...
        control_set = self.U if s2.cp_id == -1 else [u for u in self.U if u[0] == s2.cp_id]
        distances = [self.distance(s_delta, s_transform) for (cp_id, u, s_transform) in control_set]
    ...
    def plan(self):
        ...
        for i in range(self.max_iterations):
            ...
            for s_near in S_near:
                ...
                new_cost = s_near.cost + w2 if u_new_[0] == s_near.cp_id else s_near.cost + w0 + w1 + w2
                if self.close(s_new_, s_new) and self.within_cs(s_new_) and new_cost < J_min:
                    # print("reconnect--")
                    J_min = new_cost
                    s_min = s_near
                    s_add = s_new_
            ...
            for s_near in S_near:
                ...
                s_near_, u_near_ = self.steer(s_add, s_near)
                new_cost = s_add.cost + w2 if u_near_[0] == s_add.cp_id else s_add.cost + w0 + w1 + w2
                if self.close(s_near_, s_near) and self.within_cs(s_near_) and new_cost < s_near.cost:
                    # print("rewire--")
    ...
    def find_path(self):
        ...
        if node is not None:
            ...
            last_node.control = (u_new[0], u, st)
    ...
    def construct_control(self, u):
        control = {}
        control['push_x'] = self.cp_dict[u[0]][0]
        control['push_y'] = self.cp_dict[u[0]][1]
        control['push_ori'] = self.cp_dict[u[0]][2]
...
def plan(node_s, node_t, control_file=None, save_plot=None, repeat=1):
    min_cost = float('inf')
    path = []
    for i in range(repeat):
        ...
        rrt_star.set_constraints(table_size=(1.4, 1.0), object_size=(0.48, 0.33))
        # if control_type == "TshirtPushPPP":
        #     rrt_star.populate_control_space("control_pants_50cm.csv")
        # elif control_type == "ClothPushPPP":
        if control_file is None:
            rrt_star.populate_control_space("control_cloth_10cm.csv")
        else:
    ...
    path_ = [(state.pos, None if state.control is None else rrt_star.construct_control(state.control)) for state in path]
